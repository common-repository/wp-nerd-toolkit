...
// Get Plugin Settings
  ...
  if ($daily_backup == '1'){
	mkdir(ABSPATH .'wp-content/uploads/wpnerd_backup_'. date("Ymd-hi"), 0777);
  	$new_ordner = ABSPATH .'wp-content/uploads/wpnerd_backup_'. date("Ymd-hi");
...
// copy to ftp-server
if ( $ftp_backup == '1' AND isset($ftp_backup) AND !empty($ftp_backup) ){
  include( plugin_dir_path( __FILE__ ) . 'scripts/backup/ftp_upload.php');
}

// delete backup from server
if ($backup_delete == '1' AND isset($backup_delete) AND !empty($backup_delete) AND $ftp_backup == '1'){
